...
.. code-block:: protobuf
	...
	message CustomIndicatorFilter
	{
		...
		required int32 relativePosition = 3; // RelativePosition 相对位置,主要用于MA，EMA，RSI指标做比较
		optional double fieldValue = 4; // 自定义数值，用于与RSI进行比较
	...
	message StockData
	{
		...
		repeated CustomIndicatorData customIndicatorDataList = 6; // 筛选后的自定义技术指标属性数据
	};
...
.. note::
	...
	* 自定义技术指标属性筛选条件参考 `CustomIndicatorField <base_define.html#customindicatorfield>`_
	* 形态技术指标属性筛选条件参考 `PatternField <base_define.html#patternfield>`_
	* 财报时间周期参考 `FinancialQuarter <base_define.html#financialquarter>`_
	* 相对位置比较参考 `RelativePosition <base_define.html#relativeposition>`_
	* 排序方向参考 `SortDir <base_define.html#sortdir>`_
	* K线类型(仅支持K_60M，K_DAY，K_WEEK，K_MON 四种时间周期)参考 `KLType <base_define.html#kltype-k>`_
	* 限频接口：30秒内最多10次	
	* 使用类似最新价的排序字段获取数据的时候，多页获取的间隙，数据的排序有可能是变化的。
